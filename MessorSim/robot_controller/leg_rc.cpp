...
CLeg_RC::CLeg_RC(double x,double y, double z,double alpha,double beta,double gamma)
{
}

CLeg_RC::CLeg_RC(void)
{
...
void CLeg_RC::reset(double x, double y, double z, double alpha, double beta, double gamma)
{
}

/// ustawia wartosci katow w stawach
void CLeg_RC::setAngle(unsigned char joint_number,double value)
{
...
//macierz a2
	...
	a2.setElement(segment_rc_1,1,4);
...
//macierz a3
	...
	a3.setElement(segment_rc_2,1,4);
...
//macierz a4
	...
	a4.setElement(segment_rc_3,1,4);
	...
	a4.setElement(s*offset_rc_3,3,4);
...
CPunctum CLeg_RC::computeCenterOfMass(float * angles, int part){
	CPunctum temp;
	temp.setIdentity();
...
//macierz a1
	...
	a1.setElement(0.0,4,1); a1.setElement(0.0,4,2); a1.setElement(0.0,4,3); a1.setElement(1.0,4,4);
//macierz a2
	a2.setElement(cos(angles[1]),1,1);	a2.setElement(-sin(angles[1]),1,2); a2.setElement(0.0,1,3); a2.setElement(segment_rc_1,1,4);
...
//macierz a3
	a3.setElement(cos(angles[2]),1,1);	a3.setElement(-sin(angles[2]),1,2); a3.setElement(0.0,1,3); a3.setElement(segment_rc_2,1,4);
	...
	m1.setIdentity();
	m1.setElement(segment_rc_1/2.0,1,4);
	...
	m2.setIdentity();
	m2.setElement(segment_rc_2/2.0,1,4);
	...
	m3.setIdentity();
	m3.setElement(segment_rc_2/2.0,1,4);
	...
	temp.setElement((mass[0]*m1.getElement(2,4)+mass[1]*m2.getElement(2,4)+mass[2]*m3.getElement(2,4))/(mass[0]+mass[1]+mass[2]),2,4);
	temp.setElement((mass[0]*m1.getElement(3,4)+mass[1]*m2.getElement(3,4)+mass[2]*m3.getElement(3,4))/(mass[0]+mass[1]+mass[2]),3,4);
	return temp;
}

/// kinematyka odwrotna dla nogi
bool CLeg_RC::inverse_kinematic(double x, double y, double z,int part, double angles[], float scale)
{
	signed char s;
	if(part == 1){
		s=1;
	...
	angles[0]=atan2(y,x);
	double c=sqrt(pow((r-(segment_rc_1*scale)),2)+pow(z,2));
	double e=(pow(c,2)-pow(segment_rc_2*scale,2)-pow(segment_rc_3*scale,2))/(2*(segment_rc_2*scale)*(segment_rc_3*scale));
	angles[2]=atan2(-sqrt(1-pow(e,2)),e);
	double f=(segment_rc_3*scale)*sin(-angles[2]*s);
	double g=(segment_rc_3*scale)*cos(-angles[2]*s)+(segment_rc_2*scale);
	angles[1]=s*(atan2(z,r-(segment_rc_1*scale))+atan2(f,g));
...
/// kinematyka odwrotna dla nogi
bool CLeg_RC::inverse_kinematic(double x, double y, double z,int part, float angles[], float scale)
{
	signed char s;
	if(part == 1){
		s=1;
	...
	angles[1]=s*(atan2(z,r-(segment_rc_1*scale))+atan2(f,g));
	//if ((angles[2]<-3.31)||(angles[2]>-0.17)||(angles[1]>1.4)||(angles[1]<-1.57)||abs(angles[0])>1.57||(_isnan(angles[0]))||(_isnan(angles[1]))||(_isnan(angles[2])))
	if ((angles[1]>1.4)||fabs(angles[0])>1.57||(_isnan(angles[0]))||(_isnan(angles[1]))||(_isnan(angles[2])))
		return false;
	else
		return true;
}
...
bool CLeg_RC::moveFoot(double x, double y, double z, int part) 
{
	float delta_angle[3];
...
bool CLeg_RC::computeMoveDeltaAngle(double x, double y, double z, int part, float * delta_angle) 
{
...
void CLeg_RC::computeMoveDeltaAngle(double alpha, double beta, double gamma, float * delta_angle)
{
	double start_angle[3]={joint[0].getAngle(),joint[1].getAngle(),joint[2].getAngle()};
...
CPunctum CLeg_RC::getPosition(int part)
{
	int _side;
	if (part==1)
		_side = 1;
...
CPunctum CLeg_RC::getNeutralPosition(int part)
{
	int _side;
	if (part==1)
		_side = 1;
...
bool CLeg_RC::setFootPosition(double x,double y, double z,int part)
{
	CPunctum point;
	int _side;
	if (part==1)
...
float CLeg_RC::computeKinematicMargin(double alpha,double beta, double gamma,int part,int up){
...
float CLeg_RC::computeKinematicMarginApprox(double alpha,double beta, double gamma,int part,int up){
	float in[3];
...
float CLeg_RC::computeKinematicMarginPos(double x,double y, double z,int part,int up){
...
float CLeg_RC::maximizeKinematicMarginPos(double x,double y, double z,int part,int up){
...
bool CLeg_RC::isFootPositionAvailableLocal(double x,double y, double z,int part,int up, float scale)
{
	CPunctum point;
	int _side;
	if (part==1)
